 ...
 *
 * ## What it is for, and all of it is money
 *
 ...
 *   2. **The transaction id of signed bytes.** `keccak256(rawTx)` is the hash an EVM chain will
 *      know a transaction by, and deriving it rather than remembering it is what makes it
 *      available on the recovery path as well as the happy one — a node answers a re-broadcast of
 *      a transaction it already holds with an ERROR rather than the hash, so a crash between
 *      broadcasting and recording would otherwise leave a payment on chain with no id to poll.
 *   3. **EIP-191 / EIP-4361.** The digest a wallet actually signs is Keccak of the prefixed
 *      message, and recovering the signer's address is Keccak of the recovered public key. This
 *      is `wallet`'s use, and it is why a sign-in that should succeed fails if the padding byte
 *      is wrong.
 *
 * ## WHY THIS IS A PACKAGE, WHICH IS THE WHOLE POINT
 *
 * Until 2026-08-26 this file existed FIVE times: `mint`, `foresight`, `settlement` and `faucet`
 * held byte-identical copies, and `wallet` held one differing only in this comment block —
 * measured, not assumed: strip the comments and all five hash to `0772cd9f…`.
 *
 * Five copies of a permutation is not five times the risk of one; it is the risk that they stop
 * being five copies. Every one of the uses above turns a hash into an address or a payment id,
 * so a fix applied to four of five would not fail loudly. It would present as `wallet` refusing
 * a withdrawal to a destination the user copied out of our own UI, or as `settlement` polling
 * for a transaction id no node has — each looking like a bug in the service that noticed.
 *
 * So the rule this package exists to enforce: there is one permutation in this estate, every
 * service that needs a Keccak imports it from here, and no service grows a `keccak.ts` of its
 * own again.
 */
